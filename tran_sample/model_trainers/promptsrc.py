...
from clip_src.simple_tokenizer import SimpleTokenizer as _Tokenizer
...
class VLPromptLearner(nn.Module):
    def __init__(self, classnames, clip_model):
        ...
        PROMPT_DEPTH_TEXT = 9
        assert PROMPT_DEPTH_TEXT >= 1, "In Independent VL prompting, Language prompt depth should be >=1" \
        ...
        clip_model_temp = load_clip_to_cpu(True).float().to(device)
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        #         zero_shot_logits = logit_scale * zero_shot_features.to(device) @ fixed_embeddings.half().to(device).t()
