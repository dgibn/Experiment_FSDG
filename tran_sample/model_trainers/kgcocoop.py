...
class PromptLearner(nn.Module):
    def __init__(self, classnames, clip_model):
        ...
        temp = CUSTOM_TEMPLATES["Domainnet"]
...
class CustomCLIP(nn.Module):
    def __init__(self, classnames, clip_model):
        ...
        self.ori_embedding = self.prompt_learner.text_features.to(device)
        self.image_encoder = clip_model.visual
        self.text_encoder = TextEncoder(clip_model).to(device)
        self.logit_scale = clip_model.logit_scale
        self.dtype = clip_model.dtype
        self.meta_net = self.prompt_learner.meta_net.to(device)
        self.adapter = Adapter(512, 4).to(clip_model.dtype).to(device)
